...
async function getNameNumber(mthod, msg, reg) {
    ...
    let rslt_1 = rslt.UsrRegDtl;
    return toTitleCase(rslt_1);
}
// Get PIN from user
async function getPin(msg, mthod) {
    if (mthod === "password") {
        var pinRegex = /^\d{4}$/;
...
async function genrateNewID() {
    ...
    let id_array = [id_display, id_number];
    return id_array;
}
async function atm_options(user_name) {
    const rslt = await inquirer.prompt([
        {
            type: "rawlist",
