...
import { transferMSG } from "./message.js";
import chalk from "chalk";
async function get_validate_ID() {
    let chance_id = 4;
    while (true) {
        ...
        if (typeof UsrID_valid === "string") {
            ...
            --chance_id;
            if (chance_id <= 0) {
                console.log(chalk.yellow("Too many wrong attempts\nRelauch the process!!!"));
                await sleep("----------------------");
                var id_found = false;
                var return_detail = ["", 0, 0];
                break;
            }
        }
...
async function get_validate_PIN(id_found, UsrID_valid) {
    if (id_found) {
        let pin_chance = 4;
        while (pin_chance > 0) {
            --pin_chance;
...
async function transfer_amount(UserID, name_1, pin_1, amount_1) {
    ...
    if (amount_transfer <= amount_1) {
        ...
        let transfer_recipt = await transferMSG(UserID, transfer_ID, amount_transfer);
...
async function withdraw_cash(UserID, name_1, pin_1, amount_1) {
