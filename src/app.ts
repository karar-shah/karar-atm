...
import { showBalance, transferMSG } from "./message.js";
import {
    get_validate_ID,
    get_validate_PIN,
    transfer_amount,
...
async function ifAlreadyUser(): Promise<boolean> {
    return inquirer
        .prompt([
            {
                type: "checkbox",
    ...
    try {
        ...
        while (mainLoop) {
            ...
            // NO
                ...
                // get new user name
                let newUsrName = await getNameNumber(
                    "input",
                    "Enter full name:",
                    nameRegx
                );
                ...
                // get new user PIN
                while (true) {
                    var newUsrPin = await getPin(
                        "Enter 4 digit PIN",
                        "password"
                    );
                    await sleep("----------------------");
                    await sleep(undefined, 500);
                    var newUsrPin_1 = await getPin(
                ...
                var newUsrDepositAmount = await getPin(
                ...
                // display credentials
                await sleep(
                    chalk.bold.underline("Remember the following credentials")
                );
                // await sleep("----------------------")
                await sleep();
                let acc_sucess_msg = await accountCreatedMSG(
                    newUsrName,
                    newUsrId[1],
                    newUsrPin,
                    newUsrDepositAmount
